...
while cap.isOpened():
    ...
    if used_defect is not None:
        ...
        if count_defects == 2:
            ...
            cv2.circle(crop_img, (display_x, display_y),
                       5, [255, 255, 255], 20)
    ...
    if CLICK is not None:
        cv2.putText(img, CLICK_MESSAGE, (50, 200),
                    cv2.FONT_HERSHEY_SIMPLEX, 3, 3)
        if CLICK < time.time():
            CLICK = None

    cv2.putText(img, "Defects: " + str(count_defects),
                (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 2, 2)
